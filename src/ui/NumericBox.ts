...
export class NumericBox extends ControlBase {
    ...
    format() {
        ...
        if (scale > 0) {
            ...
            while (expr.test(text)) text = text.replace(expr, `$1${o.groupingSeparator}$2`);
        }
        ...
        if (n === 0) formatted = f[2].replace(placeholder, text);
        else {
            formatted = n > 0 ? f[1].replace(placeholder, text) : f[0].replace(placeholder, text.replace('-', ''));
        }
    ...
    getRawValue() {
        ...
        if (value[0] === ',' || value[0] === '.') matches.unshift('0');

        let len = matches.length;
        if (len === 1) return parseFloat(matches[0]).toString();
    ...
    private _normalizeFormat(): void {
        ...
        } else if (Array.isArray(format) && format.length === 2) format.push(format[1]);
    ...
    private _onKeyDownEvent(e: KeyboardEvent): void {
        ...
        if (/[0-9]/.test(e.key) && !e.shiftKey) return;

        let negSign = e.key === '-';
        if (
            negSign &&
            (!el.value || (el.selectionStart === 0 || (document.getSelection() || '').toString() === el.value))
        )
            return;
        ...
        if (
            /[.,]/.test(e.key) &&
            utils.isNumber(scale) &&
            scale > 0 &&
            (!/[.,]/g.test(el.value) || (document.getSelection() || '').toString() === el.value)
        )
            return;
    ...
    private _onFocusEvent(e: FocusEvent): void {
        let el = this.$element;
        if (!this.$options.nullable && !el.value) this.format();

        if (!el.hasAttribute('readonly')) el.value = this.getRawValue();
    ...
    private _ensureRange(n: number, min: number | undefined, max: number | undefined): number {
        ...
        if (hasMin && n < <Number>min) return min as number;

        if (hasMax && n > <Number>max) return max as number;
    ...
    static from(selector: string, options: NumericBoxOptions): NumericBox[] {
        return [...document.querySelectorAll(selector)].map(
            element => new NumericBox(<HTMLInputElement>element, options)
        );
    }
}
