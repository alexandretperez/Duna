...
export class SearchBox extends ControlBase {
    ...
    constructor(element: HTMLInputElement, options: SearchBoxOptions) {
        if (!options) throw new Error('SearchBox.options is required.');

        if (!options.template) throw new Error('SearchBox.options.template is required.');

        if (!options.source) throw new Error('SearchBox.source is required.');
    ...
    private _renderNoRecords() {
        ...
        } else this._containerBody.style.display = 'none';
    }
    ...
    private _readData(data: any) {
        ...
        root.split('.').forEach(property => {
            if (data.hasOwnProperty(property)) data = data[property];
            return data;
        });
    ...
    private _promiseSourceHandler(promise: (value: string) => PromiseLike<any>) {
        ...
        this._timer = window.setTimeout(() => {
            ...
            promise(encodeURIComponent(this.$element.value)).then(
                response => {
    ...
    private _normalizeValue(value: any): string {
        ...
        if (!utils.isString(value)) value = value.toString().trim();
    ...
    private _render(dataSource: any[], value: string) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            this._searchFields.forEach(field => {
                ...
                utils.noDiacritics(fieldValue).replace(expr, (match, index) => {
                    let matchesTemplate = (options.matchesTemplate as string).replace(
                        '${0}',
                        fieldValue.substr(index, match.length)
                    );
                    let strIndex = index + match.length - 1;
                    while (strIndex > index) result.splice(strIndex--, 1);
        ...
        [...this._containerBody.querySelectorAll('img[dn-src]')].forEach(img => {
            ...
            if (value && value.length > 0) (img as HTMLImageElement).src = value;
        });
    ...
    private _defineDataFilter(dataSource: any[]) {
        ...
        if (utils.isString(dataSource[0])) {
            ...
            if (!Array.isArray(fields) || !fields.length) fields = Object.keys(dataSource[0]);
            this._searchFields = fields;
        }
    }

    private _stringDataFilter(filter: DataFilterObject) {
        return this._objectDataFilter(
            Object.assign(filter, {
                dataSource: filter.dataSource.map(p => {
                    return { 0: p };
                })
            })
        );
    ...
    private _keyboardNavigate(e: KeyboardEvent) {
        ...
        if (!item) item = (this._containerBody as any)[action[0]];
    ...
    private _updateContainerScrollPosition(item: HTMLElement, isGoingDown: boolean) {
        ...
        let value = item === body.firstElementChild ? 0 : item === body.lastElementChild ? body.scrollHeight : -1;
        ...
        if (diff < 0) body.scrollTop = top - (isGoingDown ? item.offsetHeight : item.offsetHeight * 0.2);
    }

    private _onWindowScrollOrResize() {
        if (dom.isVisible(this._container)) this._updateContainerStyle();
    }
    private _onKeyDownEvent(e: KeyboardEvent) {
        if (!dom.isVisible(this._container) || this._noRecords) return;
        ...
        if (e.key === 'Enter' || e.key === 'Tab') {
            ...
            if (!item) {
                if (this._containerBody.children.length === 1) item = this._containerBody.firstElementChild;
                else return;
            }

            if (item) this._selectItem(item as HTMLElement);
    ...
    private _onInputEvent(e: KeyboardEvent) {
        ...
        if (value.length && value.length < (this.$options.minLength as number)) return;

        this._sourceHandler(this.$options.source);
        if (!value.length) this._container.style.display = 'none';
    }
    private _onBlurEvent() {
        if (!this._container.matches(':hover')) this._container.style.display = 'none';
    }
    ...
    private _updateContainerStyle() {
        ...
        if (newContainerHeight < containerHeight) this._container.style.height = newContainerHeight + 'px';

        let x = rect.left + offsetX;
        let y = bottomSize >= rect.top ? rect.bottom + offsetY : rect.top - this._container.offsetHeight - offsetY;
    ...
    private _onItemMouseEnterEvent(e: MouseEvent) {
        ...
        let currentItem = ((e.target as Element).parentElement as Element).querySelector(
            '.' + className
        ) as HTMLElement;
        if (currentItem) dom.removeClass(currentItem, className);
    ...
    static from(selector: string, options: SearchBoxOptions): SearchBox[] {
        return [...document.querySelectorAll(selector)].map(
            element => new SearchBox(<HTMLInputElement>element, options)
        );
    }
}
